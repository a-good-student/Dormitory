...
$(function (){
    ...
    document.getElementById('suit').onclick=function (){
        var p=document.getElementsByClassName('repair');
        var dormId=p[0].value, dormName=p[1].value, repairMan=p[2].value, repairName=p[3].value, repairTime=p[4].value, updateTime=p[5].value;
        $.ajax("/Dormitory/addRepair", {
            type:"post",
            dataType:"json",
            data:{dormId:dormId,dormName:dormName,repairMan:repairMan,repairName:repairName,repairTime:repairTime,updateTime:updateTime},
            // 请求成功后的回调函数。
...
function getMount() {
    var m=0;
    $.ajax("/Dormitory/repair_mount", {
        type:"post",
        dataType:"json",
        data:{dormId: dormId, dormName: dormName},
...
function setPage(num){
    $.ajax("/Dormitory/repair_import", {
        type:"post",
        dataType:"json",
        data:{num: num, dormId: dormId, dormName: dormName},
        // 请求成功后的回调函数。
        success :function(data){
            var item;
            $("#tbody").empty();//移除所有子元素
...
//删除记录
function deleteRepair(obj){
    var par=obj.parentNode.parentNode;
    $.ajax("/Dormitory/deleteRepair", {
        type:"post",
        dataType:"json",
        data:{id: par.id},
        // 请求成功后的回调函数。
...
//修改记录实际操作
function modifyRepair(obj){
    var id=parseInt(obj.name);
    var p=document.getElementsByClassName('mod');
    var dormId=p[0].value, dormName=p[1].value, repairMan=p[2].value, repairName=p[3].value, repairTime=p[4].value, updateTime=p[5].value;
    $.ajax("/Dormitory/modRepair", {
        type:"post",
        dataType:"json",
        data:{id:id,dormId:dormId,dormName:dormName,repairMan:repairMan,repairName:repairName,repairTime:repairTime,updateTime:updateTime},
        // 请求成功后的回调函数。
        success :function(data){
            //重新获取当前页
            setPage(num);
...
//表格插入数据
function setBody(repair) {
    var repairTime=repair['repairTime'].replace("T"," ");
    var updateTime=repair['updateTime'].replace("T"," ");
    var item = "<tr id=\""+repair['id']+"\">"+
        "<td>"+repair['id']+"</td>"+
        "<td>"+repair['dormId']+"</td>"+
        "<td>"+repair['dormName']+"</td>"+
        "<td>"+repair['repairMan']+"</td>"+
        "<td>"+repair['repairName']+"</td>"+
        "<td class='"+repair['repairTime']+"'>"+repairTime+"</td>"+
        "<td class='"+repair['updateTime']+"'>"+updateTime+"</td>"+
        "<td>"+"<i class='fa fa-pencil modify' onclick='modify(this);'></i>&nbsp;" +
        "&nbsp;<i class='fa fa-trash delete' onclick='deleteRepair(this);'></i>"+"</td>"+
        "</tr>";
    //设置当前页的数字标签颜色
