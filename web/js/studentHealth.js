...
$(function (){
    ...
    search_btn.onclick=function (){
        sno=search_List[0].value;
        sname=search_List[1].value;
        dormId=search_List[2].value;
        num=1;
        setPage(num);
        count=getMount();
        pageNum=Math.ceil(count/6);
    ...
    document.getElementById('suit').onclick=function (){
        ...
        var sno=p[0].value, sname=p[1].value, studentHealth=p[2].value, classId=p[3].value, dormId=p[4].value, createTime=p[5].value, updateTime=p[6].value;
        $.ajax("/Dormitory/addStudentHealth", {
            type:"post",
            dataType:"json",
            data:{sno:sno,sname:sname,studentHealth:studentHealth,classId:classId,dormId:dormId,createTime:createTime,updateTime:updateTime},
            // 请求成功后的回调函数。
...
function getMount() {
    var m=0;
    $.ajax("/Dormitory/studentHealthMount", {
        type:"post",
        dataType:"json",
        data:{sno: sno,sname: sname,dormId: dormId},
...
function setPage(num){
    $.ajax("/Dormitory/studentHealthImport", {
        type:"post",
        dataType:"json",
        data:{num: num, sno:sno, sname:sname, dormId:dormId},
        // 请求成功后的回调函数。
        success :function(data){
            var item;
            $("#tbody").empty();//移除所有子元素
...
//删除记录
function deleteHealth(obj){
    var par=obj.parentNode.parentNode;
    $.ajax("/Dormitory/deleteStudentHealth", {
        type:"post",
        dataType:"json",
        data:{id: par.id},
        // 请求成功后的回调函数。
...
//修改记录实际操作
function modifyHealth(obj){
    var id=parseInt(obj.name);
    var p=document.getElementsByClassName('mod');
    var sno=p[0].value, sname=p[1].value, studentHealth=p[2].value, classId=p[3].value, dormId=p[4].value, createTime=p[5].value, updateTime=p[6].value;
    $.ajax("/Dormitory/modStudentHealth", {
        type:"post",
        dataType:"json",
        data:{id:id,sno:sno,sname:sname,studentHealth:studentHealth,classId:classId,dormId:dormId,createTime:createTime,updateTime:updateTime},
        // 请求成功后的回调函数。
        success :function(data){
            //重新获取当前页
            setPage(num);
...
//表格插入数据
function setBody(health) {
    var createTime=health['createTime'].replace("T"," ");
    var updateTime=health['updateTime'].replace("T"," ");
    var item = "<tr id=\""+health['id']+"\">"+
        "<td>"+health['id']+"</td>"+
        "<td>"+health['sno']+"</td>"+
        "<td>"+health['sname']+"</td>"+
        "<td>"+health['studentHealth']+"</td>"+
        "<td>"+health['classId']+"</td>"+
        "<td>"+health['dormId']+"</td>"+
        "<td class='"+health['createTime']+"'>"+createTime+"</td>"+
        "<td class='"+health['updateTime']+"'>"+updateTime+"</td>"+
        "<td>"+"<i class='fa fa-pencil modify' onclick='modify(this);'></i>&nbsp;" +
        "&nbsp;<i class='fa fa-trash delete' onclick='deleteHealth(this);'></i>"+"</td>"+
        "</tr>";
    //设置当前页的数字标签颜色
