...
$(function (){
    ...
    search_btn.onclick=function (){
        classId=search_List[0].value;
        className=search_List[1].value;
        teacher=search_List[2].value;
        num=1;
        setPage(num);
        count=getMount();
        pageNum=Math.ceil(count/6);
    ...
    document.getElementById('suit').onclick=function (){
        var p=document.getElementsByClassName('cla');
        var check=document.getElementsByName('check');
        var classId=p[0].value, className=p[1].value, teacher=p[2].value;
        $.ajax("/Dormitory/addClass", {
            type:"post",
            dataType:"json",
            data:{classId: classId,className: className,teacher: teacher},
            // 请求成功后的回调函数。
...
function setPage(num){
    $.ajax("/Dormitory/cla_import", {
        type:"post",
        dataType:"json",
        data:{num: num, classId: classId,className: className,teacher: teacher},
        // 请求成功后的回调函数。
        success :function(data){
            var item;
            $("#tbody").empty();//移除所有子元素
            $.each(data,function(i,Class){
                item = setBody(Class);
                //防止页数少于3时出现bug
                if(pageNum<=3){
...
function modifyClass(obj){
    ...
    var classId=p[0].value, className=p[1].value, teacher=p[2].value;
    $.ajax("/Dormitory/modClass", {
        type:"post",
        dataType:"json",
        data:{id:id,classId: classId,className: className,teacher: teacher},
        // 请求成功后的回调函数。
        success :function(data){
            //重新获取当前页
            setPage(num);
...
//表格插入数据
function setBody(Class) {
    var item = "<tr id=\""+Class['id']+"\">"+
        "<td>"+Class['id']+"</td>"+
        "<td>"+Class['classId']+"</td>"+
        "<td>"+Class['className']+"</td>"+
        "<td>"+Class['teacher']+"</td>"+
        "<td>"+"<i class='fa fa-pencil modify' onclick='modify(this);'></i>&nbsp;" +
        "&nbsp;<i class='fa fa-trash delete' onclick='deleteClass(this);'></i>"+"</td>"+
        "</tr>";
    //设置当前页的数字标签颜色
